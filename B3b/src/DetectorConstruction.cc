...
namespace B3
{
    ...
    G4VPhysicalVolume* DetectorConstruction::Construct()
    {
        ...
        // Create multiple crystal logical volumes with different colors (similar to SiemensVisionParameterisationCrystals)
        //
        ...
        // Create multiple logical volumes with different green intensities (like SiemensVisionParameterisationCrystals)
        const G4int nColorVariants = 25; // One for each crystal in a mini-block (5x5)
        std::vector<G4LogicalVolume*> logicCrystVariants;
        
        for (G4int i = 0; i < nColorVariants; i++) {
            // Create unique name for each crystal variant to avoid sensitive detector conflicts
            G4String crystalName = "CrystalLV_" + std::to_string(i);
            auto logicCrystVariant = new G4LogicalVolume(solidCryst,  // its solid
                                                    cryst_mat,  // its material
                                                    crystalName);  // unique name
            
            // Create visual attributes with green color gradient (similar to original implementation)
            auto crystalVisAtt = new G4VisAttributes();
            // Create a more varied color pattern
            G4double green_intensity = 0.3 + 0.7 * (G4double(i) / G4double(nColorVariants - 1));
            // Add some randomness for more variation
            G4double random_factor = G4double(rand() % 100) / 500.0; // Small random variation
            green_intensity = std::min(1.0, green_intensity + random_factor);
            
            crystalVisAtt->SetColor(0.0, green_intensity, 0.0, 1.0); // Green gradient with full opacity
            crystalVisAtt->SetVisibility(true);
            crystalVisAtt->SetForceSolid(true);  // Force solid rendering instead of wireframe
            logicCrystVariant->SetVisAttributes(crystalVisAtt);
            
            logicCrystVariants.push_back(logicCrystVariant);
        }
        ...
        // Use different color variants for visual variety (similar to SiemensVisionParameterisationCrystals)
        for (G4int ix = 0; ix < crystals_per_miniblock_axial; ix++) {
            for (G4int iy = 0; iy < crystals_per_miniblock_trans; iy++) {
                ...
                // Select crystal color variant based on position (unique for each crystal in mini-block)
                G4int color_variant = copy_number; // Each crystal in mini-block gets unique color
                
                new G4PVPlacement(nullptr,  // no rotation
                                G4ThreeVector(x_pos, y_pos, 0),  // position
                                logicCrystVariants[color_variant],  // its logical volume (with unique color)
                                "Crystal",  // its name
        ...
        // Crystal colors are set individually during logical volume creation (similar to SiemensVisionParameterisationCrystals)
    ...
    void DetectorConstruction::ConstructSDandField()
    {
        ...
        // Apply to all crystal logical volume variants
        //
        ...
        // Set sensitive detector for all crystal variants
        const G4int nColorVariants = 25; // Match the number defined above
        for (G4int i = 0; i < nColorVariants; i++) {
            G4String crystalName = "CrystalLV_" + std::to_string(i);
            SetSensitiveDetector(crystalName, cryst);
        }
